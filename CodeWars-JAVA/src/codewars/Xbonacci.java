...
a fibonacci function that given a signature array/list,
...
that I really recommend to any math enthusiast and for showing me this mathematical
curiosity too with his usual contagious passion :)]
	 */
	
	public double[] tribonacci(double[] s, int n) {
		double[] arrayToReturn = new double[n];
		if (n == 0) {
		...
		for (int i = 0; i < n; i++) {
			if (i < 3) {
				...
				arrayToReturn[i] = arrayToReturn[i - 3] + arrayToReturn[i - 2] + arrayToReturn[i - 1];
			}
		}
		return arrayToReturn;
	}
